...
gamma = 0.003; % Weight for graph similarity term
...
for i = 1:iter
    ...
    cvx_begin
        variable L_hat(N, N) symmetric
        minimize(alpha * trace(Y' * L_hat * Y) + beta * square_pos(norm(L_hat, 'fro')) + gamma * trace(Y' * L_structural * Y))
        subject to
            sum(L_hat, 1).' == zeros(N, 1); % Row sums of L_hat = 0
            L_hat == L_hat.'; % L_hat is symmetric
            trace(L_hat) == N; % Trace of L_hat = N
    ...
    % Solve for Y_hat
    Y_hat = (eye(N) + alpha * L_hat + gamma * L_structural) \ X;
...
figure()
subplot(1, 3, 1);
imagesc(A_structural);
title('Graph Adjacency structural');
